...
public interface ProjectMapper {
    ...
    @Mapping(target = "projectManagerName", expression = "java(project.getProjectManager() != null ? project.getProjectManager().getFirstName() + \" \" + project.getProjectManager().getLastName() : null)")
    @Mapping(target = "memberIds", expression = "java(project.getMembers().stream().map(Employee::getId).collect(java.util.stream.Collectors.toList()))")
